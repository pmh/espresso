...
var Factory  = require('./ometa/jsonml').factory;
 ...
 *     ReturnStmt(message);
 */
register("ReturnStmt", {}, function (message) {
  this.append(message || nodes.Id("nil"));
});

/**
 * Non Local Return Statement
 * -------
 * 
 * Representation:
 *     [#NonLocalReturnStmt, {}, message]
 * 
 * Call like:
 *     NonLocalReturnStmt(message);
 */
register("NonLocalReturnStmt", {}, function (message) {
  this.append(message || nodes.Id("nil"));
});
 ...
 * 
 * Representation:
 *     [#PartialLambda, {name: string | undefined}, Expression]
 * 
 * Call like:
 *     PartialLambda(expr);
 */
register("PartialLambda", {name: undefined});
 ...
 *     Predicate(expr);
 */
register("Predicate", {}, function (pred) {
  this.append(pred || nodes.Id("true"));
});
 ...
 *
 * Representation:
 *     [#CallMsg, {}, (CallExpr | MemberMessage), ...UnaryMessage]
 * 
 * Call like:
 *     CallMsg(expr, ...args);
 */
register("CallMsg", {}, function(expr, args) {
  this.append(expr)
      .appendAll(args);
});
 ...
 *
 * Representation:
 *     [#MemberMsg, { access: ('name' | 'calculated') }, Expression, Expression?]
 * 
 * Call like:
 *     MemberMsg(expr, accessMsg).access('calculated');
 */
register("MemberMsg", { access: 'calculated', name: undefined }, function(expr, accessExpr){
 ...
 *     MethodDef(name, body);
 */
register("MethodDef", { predicates: [] }, function (lhs, rhs, pred) {
  rhs[0] = "Method"
  if (lhs.hasType("KeywordDef")) {
    var args  = nodes.FunArgs(lhs.slice(2).filter(function (arg) { return !arg.hasType("Predicate"); }))
      , preds = lhs.slice(2).filter(function (arg) { return arg.hasType("Predicate"); });
    this.predicates(preds.map(function (_pred) { return _pred[2]; }));
    rhs = nodes.Lambda(args, rhs.slice(3)[0]);
    rhs[0] = "Method"
    rhs.name(lhs.name());
    this.append(nodes.Id(lhs.name())).append(rhs);
  } else {
    if (pred) this.predicates([pred]);
    this.append(lhs, rhs);
  }
});
