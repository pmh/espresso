 ...
 * 
 * Representation:
 *     [#Method, {name: string}, FunName, FunArgs, FunBody]
 * 
 * Call like:
 *     Method(args, body).name(name);
 * or:
 *     Method(KeywordMessage(...), body);
 */
register("Method", {name: undefined}, function () {
  if (arguments[0].hasType("KeywordMsg")) {
    this.name(arguments[0].name());
    this.append(nodes.FunArgs(arguments[0].slice(2))).append(nodes.FunBody(arguments[1]));
  } else {
 ...
 *
 * Representation:
 *     [#AssignMsg, {}, Expression, Expression]
 * 
 * Call like:
 *     AssignMsg(receiver, message);
 */
register("AssignMsg");
