...
module.exports = function (_) {
  return {
    ...
    Require: function (attr) {
      ...
      try {
        ...
        if (ext === "js")
          ...
          return require('./espresso').compileFile(ext ? path : (path + ".es"));
      } catch (ex) {
    ...
    Lambda: function (attr, args, body) {
      ...
      var params = args.slice(2).map(function (node) {
        if (node.hasType("VarArg")) {
          ...
          var name = node.value();
          return join('$elf["', name, '"] = (', name, ' && ', name, '.length) ? ', name, '[0] : ', name, ' || nil;');
        }
      }).join(' ');

      return join_nl(
        '(function (' + _.translate(args) + ') {',
          this.indent('var self = this.context, $elf = self.clone(self.context);') +
          (params.length !== 0 ? ('\n' + this.indent(params)) : '') +
          (body.length   !== 0 ? ('\n' + this.indent(body))   : ''),
        '})'
      );
    ...
    AssignMsg: function (attr, id, expr) {
      if (expr.hasType("Lambda"))
        return join('["send:args:"]("set:to:", ["', id.value(), '", (function(ctx, body) { body.__context = ctx; return body; })($elf, ', this.implicit_self(_.translate(expr)) ,')]',')')
