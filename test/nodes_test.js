...
describe('Method', function () {
  describe("when called with a keyword message", function () {
    it ("should set it's name to the keyword message's name", function () {
      var token = _.Method(_.KeywordMsg([_.Keyword(_.Id("foo")), _.Id("bar"), _.Keyword(_.Id("baz")), _.Id("quux")]), []);
      token.name().should.eql('foo:baz:');
    });
    
    it ("should append the keyword message's arguments", function () {
      var token = _.Method(_.KeywordMsg([_.Keyword(_.Id("foo")), _.Id("bar"), _.Keyword(_.Id("baz")), _.Id("quux")]), []);
      token[2].should.eql(_.FunArgs([_.Id("bar"), _.Id("quux")]));
    });
    
    it ("should append the body", function () {
      var token = _.Method(_.KeywordMsg([_.Keyword(_.Id("foo")), _.Id("bar")]), [_.Id("foo"), _.Id("bar")])
      token[3].should.eql(_.FunBody([_.Id("foo"), _.Id("bar")]));
    });
  })
  
  describe ("when called with name, args and body", function () {
    it ("should set the name", function () {
      var token = _.Method(_.FunArgs([_.Id("a")]), _.FunBody([_.Id("b")])).name("foo");
      token.name().should.eql("foo");
    });
    
    it ("should append the arguments", function () {
      var token = _.Method(_.FunArgs([_.Id("a")]), _.FunBody([_.Id("b")])).name("foo");
      token[2].should.eql(_.FunArgs([_.Id("a")]));
    });
    
    it ("should append the body", function () {
      var token = _.Method(_.FunArgs([_.Id("a")]), _.FunBody([_.Id("b")])).name("foo");
      token[3].should.eql(_.FunBody([_.Id("b")]));
    });
  });
  // it ("should append all non keyword nodes", function () {
  //   var token = _.KeywordMsg([_.Keyword(_.Id("foo")), _.Id("bar"), _.Keyword(_.Id("baz")), _.Id("quux")]);
